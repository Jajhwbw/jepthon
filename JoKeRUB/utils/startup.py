    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await l313l.tgbot.send_file(
                BOTLOG_CHATID,
                "https://t.me/agnabyo/246",
                caption="**‏᯽︙ بــوت ايــڪاࢪ يـعـمـل بـنـجـاح ✓ \n᯽︙ أرسل `.الاوامر`لرؤية اوامر السورس \n  ᯽︙ لأستعمال بوت الأختراق عبر كود التيرمكس أرسل`.هاك`**",
                buttons=[(Button.url("سورس ايــڪاࢪ", "https://t.me/ucriss"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def add_bot_to_logger_group(chat_id):
    ...
    try:
        ...
        try:
            ...
            LOGS.error(str(e))
#by @ucriss بس اشوفك خامطه للكود اهينك وافضحك
JoKeRUB = {"@ucriss", "@EG_RM"}
async def saves():
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @ucriss"
        photobt = await l313l.upload_file(file="l313l/razan/resources/start/Jepthon.JPEG")
        botlog_group_id = await aljoker_the_best(l313l, "مجموعة أشعارات ايــڪاࢪ")
        if botlog_group_id:
            addgvar("PRIVATE_GROUP_BOT_API_ID", botlog_group_id)
            print("᯽︙تم العثور على مجموعة المساعدة بالفعل وإضافتها إلى المتغيرات.")
        else:
            _, groupid = await create_supergroup(
                "مجموعة أشعارات ايــڪاࢪ", l313l, Config.TG_BOT_USERNAME, descript, photobt
            )
