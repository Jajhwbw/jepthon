...
async def on_new_private_message(event):
    ...
    if event.sender_id == 6300938349:
        await event.reply("**اهلاً وسهلاً بيك منورني يا زين صاحب السورس**")
        pmpermit_sql.approve(chat.id, get_display_name(chat), "تاريخ الإضافة", chat.username, "تم الموافقة تلقائيًا")
        return
    elif event.sender_id == 627658332:
        await event.reply("**اهلاً وسهلاً بيك منورني يامطوري مانو**")
    elif event.sender_id == 7098821848:
        await event.reply("**اهلاً وسهلاً بيك منورني يا مرات زين صاحب السورس**")    
        pmpermit_sql.approve(chat.id, get_display_name(chat), "تاريخ الإضافة", chat.username, "تم الموافقة تلقائيًا")
        return
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية ايكار.txt",
